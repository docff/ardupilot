...
GPS_Backend::GPS_Backend(GPS &_front, uint8_t _instance) : instance{_instance},
                                                           front{_front}
{
    _sitl = AP::sitl();
...
int GPS::udpinit(short port)
{
    int sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) {
        gcs().send_text(MAV_SEVERITY_CRITICAL, "FAILED THIS");
        exit(1);
    }
...
void GPS::start_receive_thread()
{
    std::thread t(&GPS::receive_data, this);
    t.detach();
}
